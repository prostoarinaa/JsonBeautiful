    ...
    void xmlcode () {
        ...
        while(true)
        {
           ...
           // cout << count<< endl;
            ...
            cout << "//////////////////////////"<< endl;
            for (int i = 0; i< repeatstring.size();i++){
                cout << repeatstring[i] << endl;
            }
            timestr1 = str.substr(time1, time2);
            timestr2 = "</" + str.substr(time1+1, time2);
           // cout << timestr1<< endl; cout << timestr2<< endl;
            repeatstring.push_back(timestr2);
            
           // cout << time1+count << " " << time2 +count<< endl;
           // cout << Coordin[50].t1 << "  " <<Coordin[50].t2<< endl;
            if ((!fileOld.eof()) && (str[2] == '{' ||  str[2] == '[' || str[3] == '{' ||  str[3] == '[' || str[1] == '{' ||  str[1] == '[' || str[4] == '{' ||  str[4] == '[')) {
                //cout << str[]<< endl;
                cout << "AAA"<<repeatstring[repeatstring.size()-1]<< endl;
                fileXML<< str  ;
               // repeatstring.pop_back();
                
            }
            else if ((!fileOld.eof()) && (str[2] != '{' &&  str[2] != '[' && str[3] != '{' &&  str[3] != '[' && str[1] != '{' &&  str[1] != '[' && str[4] != '{' &&  str[4] != '['))
            {fileXML << str ;
                cout << str[2]<< endl;
                repeatstring.pop_back();
            }
